...
# 预警机
# 预警机待命阵位
AWACS_PATROL_POINT = [-80000, 0, 7500]
...
PATROL_POINT1 = [-80000, 65000, AREA_PATROL_HEIGHT]
PATROL_POINT2 = [-80000, -65000, AREA_PATROL_HEIGHT]
PATROL_POINT3 = [-80000, 35000, AREA_PATROL_HEIGHT]
PATROL_POINT4 = [-80000, -35000, AREA_PATROL_HEIGHT]
PATROL_POINT5 = [-80000, 5000, AREA_PATROL_HEIGHT]
PATROL_POINT6 = [-80000, -5000, AREA_PATROL_HEIGHT]
...
class BlueRuleAgent(Agent):
    ...
    def step(self, sim_time,obs_blue, **kwargs):
        ...
        if self.agent_state == BlueAgentState.PATROL1 and curr_time >= PATROL_TIME2:
                            ...
                            if dis < 35000:
                                ...
                                for airports in obs_blue['airports']:
                                    ...
                                    if self.airport_flag is False and self.airport_flag_time == 9000 and self.tm_a2a < 6:
                                        cmd_list.extend(self._takeoff_areapatrol(1, 11, [-50000,0,7000], PATROL_PARAMS))
                                        print('蓝方派出初次替补编队进行替补,序号为：'+ str(self.tm_a2a + 1)+',替补位置为'+str(point_T))
                                        self.tm_a2a += 1

                # 获取红方舰船
                if red_unit['LX'] == 21 and red_unit['ID'] not in self.target_ship_list:
                    cmd_list.extend(
                        self._takeoff_targethunt(2, red_unit['ID'], [90, 80, 160]))
                    self.target_ship_list.append(red_unit['ID'])
                    print('蓝方派出2架轰炸机攻击红方护卫舰，地面轰炸机数量还剩'+str(obs_blue['airports']['BOM']))
                # # 获取红方机场
                # if red_unit['LX'] == 42:
                #     cmd_list.extend(self._takeoff_areahunt(3, [red_unit['X'], red_unit['Y'], 5000], [90, 1000, 1000, 160]))
        # 如果没有情报就按计划区域巡逻
        else:
            self.target_list = []
    ...
    def _takeoff_targethunt(num, target_id, takeoff_targethunt_params):
        return [EnvCmd.make_takeoff_targethunt(BLUE_AIRPORT_ID, num, target_id, *takeoff_targethunt_params)]

    # 轰炸机护航
    def _A2G_escort(self, a2g_team_id):
        return [EnvCmd.make_takeoff_protect(BLUE_AIRPORT_ID, 2, a2g_team_id, 1, 100, 250)]
    # 护卫舰区域巡逻
    def _ship_areapatrol(self, self_id, point):
        return [EnvCmd.make_ship_areapatrol(self_id, *point, 180,20000,2000,8,14400,0)]