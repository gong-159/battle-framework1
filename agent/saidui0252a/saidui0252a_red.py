...
# 护卫舰
# 北部巡逻阵位
SHIP_POINT1 = [5000, 5000, 0]
# 中部巡逻阵位
SHIP_POINT2 = [5000, -10000, 0]
# 南部巡逻阵位
SHIP_POINT3 = [25000, -65000, 0]
SHIP_PATROL_PARAMS_0 = [180, 20000, 20000, 8, 14400, 0]
...
# 预警机
# 预警机待命阵位
AWACS_PATROL_POINT = [30000, 0, AWACS_PATROL_HEIGHT]
# 预警机南部规避阵位
...
# 空中干扰北部干扰阵位
AIR_DISTURB_POINT2 = [-45000, 65000, DISTURB_PATROL_HEIGHT]
# 空中干扰北部干扰规避阵位
AIR_DISTURB_POINT3 = [-25000, 55000, DISTURB_PATROL_HEIGHT]
# 空中干扰南部干扰阵位
AIR_DISTURB_POINT4 = [-45000, -55000, DISTURB_PATROL_HEIGHT]
# 空中干扰中部干扰阵位
AIR_DISTURB_POINT6 = [0, -55000, DISTURB_PATROL_HEIGHT]
# 空中干扰南部干扰规避阵位
AIR_DISTURB_POINT5 = [-25000, -55000, DISTURB_PATROL_HEIGHT]

AIR_DISTURB_POINT7 = [-40000, -55000, DISTURB_PATROL_HEIGHT]

AIR_DISTURB_POINT8 = [-15000, 0, DISTURB_PATROL_HEIGHT]

#轰炸机起飞目标突击参数
takeoff_target_hunt_params = [270, 80, 160]
...
NORTH_LINE = [Point(45000, 0, 8500), Point(20000, -15000, 8500), Point(-25000, 55000, 8500), Point(-40000, 60000, 8500)]
# 南部航线干扰
# SOUTH_LINE = [{'x':-45000,'y':55000,'z':8500},{'x':-45000,'y':-55000,'z':8500}]
SOUTH_LINE = [Point(-45000, 55000, 8500), Point(-45000, -55000, 8500)]

MID_LINE = [Point(-30000, 55000, 8500), Point(-15000, 0, 8500), Point(-40000, -55000, 8500)]
...
# 北部警戒阵位1(-65000, 25000)--距离北部突击阵位约40km
AIR_PATROL_POINT1 = [-15000, 35000, AIR_PATROL_HEIGHT]
# 北部警戒阵位2(-95000, 55000)--距离北部突击阵位约40km
AIR_PATROL_POINT2 = [-55000, -45000, AIR_PATROL_HEIGHT]
# 中部阻援/集结阵位(-105000, 0)--距离南/北部干扰阵位均约78km, 可伏击敌增援北岛战机, 距离南岛约90km, 也是南下的集结阵位
AIR_PATROL_POINT3 = [0, 40000, AIR_PATROL_HEIGHT]
...
AIR_PATROL_POINT5 = [-55000, 0, AIR_PATROL_HEIGHT]
# 南部警戒阵位2(-105000, -65000)--南岛东北方向约37km
AIR_PATROL_POINT6 = [-55000, 45000, AIR_PATROL_HEIGHT]
# 南部警戒阵位3(-95000, -85000)--南岛东侧约37km, 距离己南部干扰阵位60km
AIR_PATROL_POINT7 = [-95000, -85000, AIR_PATROL_HEIGHT]

AIR_PATROL_POINT8 = [-30000, 45000, AIR_PATROL_HEIGHT]
AIR_PATROL_POINT9 = [-55000, 10000, AIR_PATROL_HEIGHT]
AIR_PATROL_POINT10 = [-55000, 20000, AIR_PATROL_HEIGHT]
AIR_PATROL_POINT11 = [-55000, -20000, AIR_PATROL_HEIGHT]

#护卫舰巡逻阵位、参数
SHIP_PATROL_POINT1 = [-10000, 45000, 0]
SHIP_PATROL_PARAMS1 = [180, 10000, 10000, 15, 14400, 0]

SHIP_PATROL_POINT2 = [-10000, -45000, 0]
SHIP_PATROL_PARAMS2 = [180, 10000, 10000, 15, 14400, 0]
...
class RedRuleAgent(Agent):
    ...
    def _init(self):
        ...
        self.attackFrigateTime = 0
        self.returnTime = {}
        self.takeOffAgainAndAttack = 1
        self.attackNorthTime = 0
        self.attackSouthTime = 0
    ...
    def step(self, sim_time, obs_red, **kwargs):
        ...
        # print(sim_time)
        # if   sim_time >=300 and sim_time<=310 :
        #     print('111111111111111111111111111111111222222222222222222222')
        #     cmd_list.extend([EnvCmd.make_simulation('PAUSE','', 5)])
        '''第一波次'''
        # 护卫舰初始化
        ...
        if self.disturb_state == RedAgentState.AIR_DISTURB2:
            for disturb in obs_red['units']:
                if disturb['LX'] == 13 and -45000 < disturb['X'] < -35000 and 50000 < disturb['Y'] < 70000:
                    cmd_list.extend(
            ...
            if sim_time > 2520:
                for disturb in obs_red['units']:
                    if disturb['LX'] == 13 and 'TMID'in disturb.keys():
                        # cmd_list.extend(self._disturb_patrol(self.team_id_dic['RA'], AIR_DISTURB_POINT8, DISTURB_PATROL_PARAMS))
                        cmd_list.extend(self._disturb_linepatrol(self.team_id_dic['RA'], MID_LINE))
                        print('干扰机南下干扰')
        ...
        if self.disturb_state == RedAgentState.AIR_DISTURB4:
            for disturb in obs_red['units']:
                if disturb['LX'] == 13 and -20000 < disturb['X'] < -10000 and -10000 < disturb['Y'] < 10000:
                # if disturb['LX'] == 13 and -50000 < disturb['X'] < -40000 and -60000 < disturb['Y'] < -50000:
                    cmd_list.extend(
                        self._disturb_patrol(self.team_id_dic['RA'], AIR_DISTURB_POINT7, DISTURB_PATROL_PARAMS))
                    self.disturb_state = RedAgentState.AIR_DISTURB5

        if self.disturb_state == RedAgentState.AIR_DISTURB5:
            for disturb in obs_red['units']:
                if disturb['LX'] == 13 and sim_time > 5000 and 'TMID'in disturb.keys():
                # if disturb['LX'] == 13 and -50000 < disturb['X'] < -40000 and -60000 < disturb['Y'] < -50000:
                    cmd_list.extend(
                        self._disturb_patrol(self.team_id_dic['RA'], AIR_DISTURB_POINT6, DISTURB_PATROL_PARAMS))
                    self.disturb_state = RedAgentState.AIR_DISTURB6

        # 轰炸机4机编队--HA + 护航歼击机2机编队--JC
        if self.agent_state == 15:
            cmd_list.extend(self._takeoff_areapatrol(4, 15, AREA_HUNT_POINT1, AREA_PATROL_PARAMS))
            print('轰炸机HA起飞')
            self.attackFrigateTime = sim_time
            ...
            for ship in obs_red['units']:
                if ship['LX'] == 21:
                    if shipIndex == 1:
                        cmd_list.extend(self._ship_areapatrol(ship['ID'], SHIP_PATROL_POINT1))
                        shipIndex += 1
                        continue
                    if shipIndex == 2:
                        cmd_list.extend(self._ship_areapatrol(ship['ID'], SHIP_PATROL_POINT2))
                        continue
            self.agent_state = 9
        ...
        if self.agent_state == 9:
            ...
            # for ship in obs_red['qb']:
            #     if ship['LX'] == 21 and 'HA' in list(
            #             self.team_id_dic.keys()) and self.area_hurt_a == RedAgentState.AREA_HUNT11:
            #         cmd_list.extend(self._targethunt(self.team_id_dic['HA'], ship['ID']))
            #         self.area_hurt_a = RedAgentState.AREA_HUNT12
            #         print('HA进行目标突击，目标为蓝方舰船')
            # self.attackFrigateTime = sim_time
                ...
                if 'TMID'in a2g.keys() and a2g['TMID'] == self.team_id_dic['HA'] and -75000 < a2g['X'] < -35000 and 50000 < a2g['Y'] < 80000:
                    for a2a in obs_red['units']:
                        if a2a['TMID'] == self.team_id_dic['JC']:
                            cmd_list.extend(self._areapatrol(a2a['ID'], AIR_PATROL_POINT8, AIR_PATROL_PARAMS_0))
                            print('JC进入北部警戒阵位1')
                    self.a2g_ha = 1

        # 轰炸机6机编队--HB + 护航歼击机2机编队--JD
        if self.agent_state == 10 and sim_time - self.attackFrigateTime > 400:
            cmd_list.extend(self._takeoff_areahunt(2, AREA_HUNT_POINT0))
            cmd_list.extend(self._takeoff_areahunt(4, AREA_HUNT_POINT0))

            self.attackNorthTime = sim_time

            # for ship in obs_red['qb']:
            #     if ship['LX'] == 21:
            #         cmd_list.extend(self._takeoff_targethunt(2, ship['ID'], takeoff_target_hunt_params))

            # for zhs in obs_red['qb']:
            #     if zhs['LX'] == 41:
            #         cmd_list.extend(self._takeoff_targethunt(2, zhs['ID'], takeoff_target_hunt_params))
            #         cmd_list.extend(self._takeoff_targethunt(4, zhs['ID'], takeoff_target_hunt_params))
            #         break
            print('轰炸机HB起飞')
            self.agent_state = 11
                ...
                if 'TMID'in a2g.keys() and a2g['TMID'] == self.team_id_dic['HB'] and -75000 < a2g['X'] < -35000 and 50000 < a2g['Y'] < 80000:
                    for a2a in obs_red['units']:
                        if a2a['TMID'] == self.team_id_dic['JD']:
                            cmd_list.extend(self._areapatrol(a2a['ID'], AIR_PATROL_POINT9, AIR_PATROL_PARAMS_0))
                    self.a2g_hb = 1

        # 阻援歼击机2机编队--JE
        if self.agent_state == 8:
            cmd_list.extend(self._takeoff_areapatrol(2, 11, AIR_PATROL_POINT10, AIR_PATROL_PARAMS))
            print('阻援歼击机JE起飞')
            self.agent_state = 13
        # 歼击机JG, JH
        if self.agent_state == 13:
            cmd_list.extend(self._takeoff_areapatrol(2, 11, [-45000, 65000, 8000], AIR_PATROL_PARAMS))
            print('阻援歼击机JG起飞')
            self.agent_state = 14
        if self.agent_state == 14:
            cmd_list.extend(self._takeoff_areapatrol(2, 11, [-106700, -10000, 8000], AIR_PATROL_PARAMS))
            print('防守歼击机JH起飞')
            self.agent_state = 15
        '''第二波次'''
        ...
        if self.agent_state == 17:
            cmd_list.extend(self._takeoff_areapatrol(2, 11, AIR_PATROL_POINT11, AIR_PATROL_PARAMS))
            print('阻援歼击机JK起飞')
            self.agent_state = 18
        # 轰炸机4机编队--HD + 护航歼击机2机编队--JL
        if self.agent_state == 18:
            # cmd_list.extend(self._takeoff_areahunt(2, AREA_HUNT_POINT4_0))
            cmd_list.extend(self._takeoff_areahunt(4, AREA_HUNT_POINT4_0))

            self.attackSouthTime = sim_time
        ...
        for blue_target in self.blue_list:
            ...
            if blue == 0 or del_blue is False or del_blue2 is False:
                ...
                for a2a in obs_red['units']:
                    ...
                    if attack == 0:
                        if a2a['LX'] == 11 and a2a['ID'] not in list(self.blue_dic.keys()):
                            if int(a2a['WP']['170']) > 0:
                                for blueid in obs_red['qb']:
                                    if blue_target == blueid['ID']:
                                        cmd_list.extend(self._airattack(a2a['ID'], blue_target))
                                attack = 1
        ...
        for unit in obs_red['units']:
            if unit['LX'] == 11:
                if int(unit['WP']['170']) == 0:
                    cmd_list.extend(self._returntobase(unit['ID']))
                    self.returnTime[unit['ID']] = sim_time
            # if unit['LX'] == 15:
            #     if int(unit['WP']['360']) == 0:
            #         cmd_list.extend(self._returntobase(unit['ID']))
            #         self.returnTime[unit['ID']] = sim_time

        for unit in obs_red['units']:
            #如果机场还有歼击机，则派出来巡逻
            if unit['ID'] not in self.returnTime.keys():
                if unit['ST'] == 8:
                    if unit['LX'] == 11:
                        r = rd.random()
                        if r < 0.33:
                            cmd_list.extend(self._areapatrol(unit['ID'], [-20000, 45000, 8000], AIR_PATROL_PARAMS_0))
                        if 0.33 <= r < 0.66:
                            cmd_list.extend(self._areapatrol(unit['ID'], [-20000, 0, 8000], AIR_PATROL_PARAMS_0))
                        if 0.66 <= r:
                            cmd_list.extend(self._areapatrol(unit['ID'], [-20000, -45000, 8000], AIR_PATROL_PARAMS_0))
                    # if unit['LX'] == 15:
                    #     for zhihuisuo in obs_red['qb']:
                    #         if zhihuisuo['LX'] == 41:
                    #             cmd_list.extend(
                    #                 self._takeoff_targethunt(1, zhihuisuo['ID'], takeoff_target_hunt_params))
            else:
                if unit['ST'] == 8:
                    if sim_time - self.returnTime[unit['ID']] > 2400:
                        # 对于返航后的歼击机，如果补给时间足够，则再次派出来巡逻
                        if unit['LX'] == 11:
                            r = rd.random()
                            if r < 0.33:
                                cmd_list.extend(
                                    self._areapatrol(unit['ID'], [-20000, 45000, 8000], AIR_PATROL_PARAMS_0))
                            if 0.33 <= r < 0.66:
                                cmd_list.extend(self._areapatrol(unit['ID'], [-20000, 0, 8000], AIR_PATROL_PARAMS_0))
                            if 0.66 <= r:
                                cmd_list.extend(
                                    self._areapatrol(unit['ID'], [-20000, -45000, 8000], AIR_PATROL_PARAMS_0))
                        # 对于返航后的轰炸机，如果补给时间足够，则再次派出来攻击蓝方指挥所
                        # if unit['LX'] == 15:
                        #     for zhihuisuo in obs_red['qb']:
                        #         if zhihuisuo['LX'] == 41:
                        #             if zhihuisuo['Y'] > 0:
                        #                 cmd_list.extend(self._takeoff_areahunt(1, AREA_HUNT_POINT0))
                        #             else:
                        #                 cmd_list.extend(self._takeoff_areahunt(1, AREA_HUNT_POINT4_0))

        for bomber in obs_red['units']:
            if bomber['LX'] == 15:
                if int(bomber['WP']['360']) == 0:
                    cmd_list.extend(self._returntobase(unit['ID']))

        for airport in obs_red['airports']:
            if airport['BOM'] > 0 and self.attackNorthTime != 0 and self.attackSouthTime != 0:
                for commandPost in obs_red['qb']:
                    if commandPost['LX'] == 41:
                        if commandPost['Y'] > 0:
                            cmd_list.extend(self._takeoff_areahunt(airport['BOM'], AREA_HUNT_POINT0))
                        else:
                            cmd_list.extend(self._takeoff_areahunt(airport['BOM'], AREA_HUNT_POINT4_0))
                        break

        #轰炸机返航补给后再下攻击指令
        # for bomber in obs_red['units']:
        #     if bomber['ST'] == 8 and bomber['LX'] == 15:
        #         if sim_time - self.attackNorthTime > 6000 and self.attackNorthTime != 0:
        #             for commandPost in obs_red['qb']:
        #                 if commandPost['LX'] == 41:
        #                     if commandPost['Y'] > 0:
        #                         cmd_list.extend(self._takeoff_areahunt(1, AREA_HUNT_POINT0))
        ...
        #                         cmd_list.extend(self._takeoff_areahunt(1, AREA_HUNT_POINT4_0))
        #                         # self.attackSouthTime = sim_time
        #         if sim_time - self.attackSouthTime > 6000 and self.attackSouthTime != 0:
        #             for commandPost in obs_red['qb']:
        #                 if commandPost['LX'] == 41:
        #                     if commandPost['Y'] > 0:
        #                         cmd_list.extend(self._takeoff_areahunt(1, AREA_HUNT_POINT0))
    ...
    def _parse_teams(self, red_dict):
        for team in red_dict['teams']:
            ...
            # 轰炸机编队
                ...
                if 'fly_num' in list(self.Task.keys()):
                    # HA
                    if self.Task['fly_num'] == 4 and self.Task['point_x'] == -55000 and self.Task['point_y'] == 65000:
    ...
    def _takeoff_targethunt(num, target_id, takeoff_targethunt_params):
        return [EnvCmd.make_takeoff_targethunt(RED_AIRPORT_ID, num, target_id, *takeoff_targethunt_params)]
